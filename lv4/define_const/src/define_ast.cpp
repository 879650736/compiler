...
public:
    ...
    using LValPtr = std::unique_ptr<LValAST>;
    using NumPtr = std::unique_ptr<NumberAST>;

    // 处理三种可能的表达式
    std::variant<ExpPtr, LValPtr, NumPtr> expr;

    void Dump(std::ostream &out) const override {
        std::visit([&out](auto &&arg) {
            using T = std::decay_t<decltype(arg)>;
            if constexpr (std::is_same_v<T, ExpPtr>) {
                brackets_number += 2;
                vec.emplace_back("(");  // 添加左括号
                arg->Dump(out);         // 处理 ExpAST
                vec.emplace_back(")");  // 添加右括号
            } else if constexpr (std::is_same_v<T, LValPtr>) {
                arg->Dump(out);         // 处理 LValAST
            } else if constexpr (std::is_same_v<T, NumPtr>) {
...
public:
    ...
    using ConstDefPtr = std::unique_ptr<BaseAST>;

    // 表示常量声明：类型和常量定义列表
    std::vector<ConstDefPtr> const_defs;

    // 打印 AST 节点，使用 vector 存储多个 ConstDef
    void Dump(std::ostream &out = std::cout) const override {
        out << "const ";
        btype->Dump(out);  // 打印类型
...
public:
    using ConstDefPtr = std::unique_ptr<BaseAST>; // 定义常量定义指针类型
    ...
    void Dump(std::ostream &out = std::cout) const override {
        for (size_t i = 0; i < const_defs.size(); ++i) {
            const_defs[i]->Dump(out);  // 打印每个常量定义
...
public:
    std::vector<std::unique_ptr<BaseAST>> block_items;  // 使用 vector 存储多个 BlockItem

    void Dump(std::ostream &out = std::cout) const override {
        out << "%" << "entry:" << "\n";
        for (const auto &block_item : block_items) {
            block_item->Dump(out);  // 打印每个 BlockItem
        }
    }
};
...
public:
    ...
    // 表示 BlockItem 的两种情况：Decl 或 Stmt
    std::variant<DeclPtr, StmtPtr> expr;

    void Dump(std::ostream &out = std::cout) const override {
        std::visit([&out](auto &&arg) {
            using T = std::decay_t<decltype(arg)>;
            if constexpr (std::is_same_v<T, DeclPtr>) {
                arg->Dump(out);  // 处理 DeclAST
            } else if constexpr (std::is_same_v<T, StmtPtr>) {
                arg->Dump(out);  // 处理 StmtAST
            }
        }, expr);
    }
};
